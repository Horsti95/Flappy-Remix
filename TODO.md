...
2. ~~`feat/daily-aftermath`~~ — **shipped in 0.23.0.** The daily's third
   attempt now routes to `ui/daily-aftermath`, with the pasteable emoji block
   generated by `game/daily-share` (pure + unit-tested) and per-attempt scores
   kept by `game/daily-history`. No schema change was needed.
